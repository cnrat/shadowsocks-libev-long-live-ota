                  ...
                  server_t *server)
{
    ...
    if (fast_open)
    {
        ...
        if (s == -1)
        {
            if (errno == CONNECT_IN_PROGRESS || errno == EAGAIN || errno == EWOULDBLOCK)
            {
...
server_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    else if (server->stage == STAGE_PARSE)
    {
        ...
        if (accesslog)
        {
            ...
            if (peer_name != NULL)
            {
                LOGE("[ACCESS] %s -> %s:%d", peer_name, host, ntohs(port));
            }
        }
...
{
    server_ctx_t *server_ctx = cork_container_of(watcher, server_ctx_t, watcher);
    ...
    if (server->stage < STAGE_PARSE)
    {
        if (verbose)
        {
            size_t len = server->stage ? server->header_buf->len : server->buf->len;
#ifdef __MINGW32__
...
accept_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (peer_name != NULL)
    {
        ...
        if (acl)
        {
            if ((get_acl_mode() == BLACK_LIST && acl_match_host(peer_name) == 1) || (get_acl_mode() == WHITE_LIST && acl_match_host(peer_name) >= 0))
            {
